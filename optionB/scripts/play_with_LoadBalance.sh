...
TERRAFORM_DIR="$(dirname "$0")/../../terraform/environments/small/option-b"
...
echo ""
echo "NOTE: CodeDeploy will override this distribution when a new deployment runs."
echo ""
...
if [ "$TG_JSON" = "null" ] || [ -z "$TG_JSON" ]; then
  # Listener uses simple forward (not weighted) — one TG gets 100%
  ...
  if [ "$ACTIVE_TG" = "$BLUE_TG" ]; then
    BLUE_WEIGHT=100; GREEN_WEIGHT=0
  else
    BLUE_WEIGHT=0; GREEN_WEIGHT=100
  fi
else
...
echo "  1) 100% Blue  /   0% Green  (full rollback to blue)"
echo "  2)  80% Blue  /  20% Green"
echo "  3)  50% Blue  /  50% Green"
echo "  4)  20% Blue  /  80% Green"
echo "  5)   0% Blue  / 100% Green  (full cutover to green)"
echo "  6) Custom"
echo ""
